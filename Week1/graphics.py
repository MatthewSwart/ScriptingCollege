...
def _tk_thread():
    ...
    _root = tk.Tk()
    _root.withdraw()
    _root.after(POLL_INTERVAL, _tk_pump)
    _root.mainloop()

def _tk_pump():
    global _exception_info
    while not tk_request.empty():
        command = tk_request.get()
        try:
            tk_result.put(command())
        except Exception, info:
            tk_result.put(None) # release client
            _exception_info = info
    #_root.update()
    _root.after(POLL_INTERVAL, _tk_pump)

def tkExec(f, *args, **kw):
    # execute f in the Tk thread all code manipulating TK
    #   must be called through this.
    # call to tkExec in Tk thread == DEADLOCK !
    global _exception_info
    _exception_info = None
    def func():
        return f(*args, **kw)
    tk_request.put(func,True)
    result = tk_result.get(True)
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        tkExec(self.__init_help, title, width, height, autoflush)
    ...
    def plot(self, x, y, color="black"):
        ...
        tkExec(self.create_line,xs,ys,xs+1,ys,fill=color)
        self.__autoflush()
    ...
    def plotPixel(self, x, y, color="black"):
        ...
        tkExec(self.create_line, x,y,x+1,y, fill=color)
        self.__autoflush()
...
class GraphicsObject:
    ...
    def draw(self, graphwin):
        ...
        self.id = tkExec(self._draw, graphwin, self.config)
    ...
    def undraw(self):
        ...
        if not self.canvas.isClosed():
            ...
            tkExec(self.canvas.delete, self.id)
    ...
    def move(self, dx, dy):
        ...
        if canvas and not canvas.isClosed():
            ...
            tkExec(self.canvas.move, self.id, x, y)
            if canvas.autoflush:
                #_root.update()
                tkExec(_root.update)
    ...
    def _reconfig(self, option, setting):
        ...
        if self.canvas and not self.canvas.isClosed():
            ...
            tkExec(self.canvas.itemconfig, self.id, options)
            if self.canvas.autoflush:
                #_root.update()
                tkExec(_root.update)
...
class Entry(GraphicsObject):
    ...
    def __init__(self, p, width):
        ...
        #self.text.set("")
        self.text = tkExec(tk.StringVar, _root)
        tkExec(self.text.set, "foo")
        self.fill = "gray"
        self.color = "black"
    ...
    def clone(self):
        ...
        return tkExec(self.__clone_help, other)
    ...
    def setText(self, t):
        #self.text.set(t)
        tkExec(self.text.set, t)
            
    def setFill(self, color):
        self.fill = color
        if self.entry:
            #self.entry.config(bg=color)
            tkExec(self.entry.config,bg=color)
    ...
    def _setFontComponent(self, which, value):
        ...
        if self.entry:
            ...
            tkExec(self.entry.config, font=self.font)
    ...
    def setTextColor(self, color):
        ...
        if self.entry:
            ...
            tkExec(self.entry.config,fg=color)
